...
def create_labeled_unlabeled_partitions_indices(datasetpath, percentage_labeled_observations, num_batches = 10, create_dirs = False):
    ...
    base_dir = "/media/Data/saul/Code_Projects/MeanTeacher_InBreast/data-local/labels/InBreast/"
    current_folder = base_dir + "percentage_labeled_" + str(percentage_labeled_observations)
    if (create_dirs):
    ...
    for current_batch in range(0, num_batches):
        ...
        X_train, X_test, y_train, y_test = train_test_split(list_file_names, list_labels, test_size = percentage_labeled_observations, random_state = random_state)
        #X_test and y_test are the labeled data and its labels
        labeled_observations = X_test
        labeled_labels = y_test
        #"/media/Data/saul/Code_Projects/MeanTeacher_InBreast/data-local"
        #current file path
        current_file_path = current_folder + "/batch_" + str(current_batch) + ".txt"
        file = open(current_file_path, 'w')
        for i in range(0, len(labeled_observations)):
            line = labeled_observations[i] + " " + str(labeled_labels[i]) + "\n"
            file.write(line)
        file.close()
