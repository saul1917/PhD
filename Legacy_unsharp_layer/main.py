...
from aimodel import *
from CrossEntropyLossPKSoftmax import *
...
def main():
    ...
    if use_cuda:
        hierarchy_matrix = torch.cuda.FloatTensor(hierarchy_matrix)
    else:
    ...
    print(dset.hierarchy)

    model = create_model(predefined_model(args), num_classes, use_cuda)
    criterion = nn.CrossEntropyLoss().cuda() #hierarchical_log_loss
    #criterion = CrossEntropyLossPKSoftmax(hierarchy_matrix=hierarchy_matrix, cuda=use_cuda)
